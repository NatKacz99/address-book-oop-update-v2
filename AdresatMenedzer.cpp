...
void AdresatMenedzer::zaktualizujDaneWybranegoAdresata(Adresat adresat)
{
    ...
    cout << "Dane adresata zostaly edytowane." << endl;
}
int AdresatMenedzer::podajIdWybranegoAdresata()
{
    int idWybranegoAdresta = 0;
    cout << "Podaj id adresata: " << endl;
    return idWybranegoAdresta = MetodyPomocnicze::wczytajLiczbeCalkowita();
}
...
void AdresatMenedzer::usunAdresata()
{
    ...
    for (vector <Adresat>::iterator itr = adresaci.begin(); itr != adresaci.end(); itr++)
    {
        if (itr -> pobierzId() == idUsuwanegoAdresata)
        {
            ...
            if (znak == 't')
            {
                plikZAdresatami.usunLinieZDanymiWybranegoAdresata(*itr);
                cout << endl << endl << "Szukany adresat zostal USUNIETY" << endl << endl;
                system("pause");
            }
            else
            {
                cout << endl << endl << "Nie potwierdziles usuniecia. Wybrany adresat NIE zostal usuniety" << endl << endl;
                system("pause");
            }
        }
    }
